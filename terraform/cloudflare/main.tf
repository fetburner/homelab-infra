...
resource "cloudflare_zero_trust_tunnel_cloudflared" "terraform_managed_resource_console_kagawa" {
  account_id = "abdb81a14388bfb3b2ccfe00525c16df"
  config_src = "cloudflare"
  name       = "console-kagawa"
}

resource "cloudflare_zero_trust_tunnel_cloudflared" "terraform_managed_resource_epgstation" {
  account_id = "abdb81a14388bfb3b2ccfe00525c16df"
  config_src = "cloudflare"
  name       = "epgstation"
}

resource "cloudflare_zero_trust_tunnel_cloudflared" "terraform_managed_resource_k3s" {
  account_id = "abdb81a14388bfb3b2ccfe00525c16df"
  config_src = "cloudflare"
  name       = "k3s"
}
...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "console_kagawa" {
  ...
  config = {
    ingress = [
      {
        hostname       = "console-kagawa.fetburner.dev"
...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "epgstation" {
  ...
  config = {
    ingress = [
      {
        hostname       = "epgstation.fetburner.dev"
...
resource "cloudflare_zero_trust_tunnel_cloudflared_config" "k3s" {
  ...
  config = {
    ingress = [
      {
        hostname       = "epgstation-qa.fetburner.dev"
        service        = "http://nginx.epgstation.svc.cluster.local"
        origin_request = {}
      },
      {
        hostname       = "rancher.fetburner.dev"
...
resource "cloudflare_dns_record" "mx_route1" {
  ...
  ttl      = 1
  type     = "MX"
  zone_id  = "cb1cd8b1395576b4d6d7c15154897f93"
  settings = {}
}
...
resource "cloudflare_dns_record" "txt_dkim" {
  ...
  name     = "cf2024-1._domainkey.fetburner.dev"
  proxied  = false
  tags     = []
  ttl      = 1
  type     = "TXT"
